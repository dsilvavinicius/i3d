...
if __name__ == "__main__":
    ...
    if args.input in model_map:
        ...
        if not osp.exists(mesh_path := osp.expanduser(args.input)):
    ...
    if "rbf" in args.methods:
        ...
        print(f"RBF Results: SABSE {errs.sum():.3} --- MABSE {errs.mean():.3} -- MAXERR {errs.max().item():.3} --- Runtime {tot_time} s")
    ...
    if "i3d" in args.methods:
        ...
        if args.i3d_config is not None and osp.exists(path := osp.expanduser(args.i3d_config)):
        ...
        with torch.no_grad():
            ...
            print(f"i3d Results: SABSE {errs.sum():.3} --- MABSE {errs.mean():.3} -- MAXERR {errs.max().item():.3} --- Runtime {tot_time} s")
