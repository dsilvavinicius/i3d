...
from i3d.loss_functions import true_sdf, loss_sdf_on_neighborhood
...
def create_training_data_on_neighborhood(
    ...
    offset_pts_in  = surf_pts[..., :3] - delta*surf_pts[..., 3:6]
    offset_pts_out = surf_pts[..., :3] + delta*surf_pts[..., 3:6]

    pts     = torch.cat((surf_pts[..., :3] , offset_pts_in, offset_pts_out), dim=0)
    normals = torch.cat((surf_pts[..., 3:6], surf_pts[..., 3:6], surf_pts[..., 3:6]), dim=0)
    sdfs    = torch.cat((torch.zeros_like(surf_pts[..., -1]) , -delta*torch.ones_like(surf_pts[..., -1]), delta*torch.ones_like(surf_pts[..., -1])), dim=0)

    # pts     = torch.cat((surf_pts[..., :3] , surf_pts[..., :3], surf_pts[..., :3]), dim=0)
    # normals = torch.cat((surf_pts[..., 3:6], surf_pts[..., 3:6], surf_pts[..., 3:6]), dim=0)
    # sdfs    = torch.cat((torch.zeros_like(surf_pts[..., -1]) , torch.zeros_like(surf_pts[..., -1]), torch.zeros_like(surf_pts[..., -1])), dim=0)

    
    coord_dict = {
        "on_surf": [pts, normals, sdfs]
    }
...
if __name__ == "__main__":
    ...
    for step in range(nsteps):
        ...
        y = model(trainingpts)
        loss = loss_sdf_on_neighborhood(y, gt)
